            ...
            var _eventHandler = function (type, data) {
                switch (type) {
                    ...
                    case 9:
                        //Инициируем субтитры
                        _initSubtitle();
                        //Получаем информацию о встроенных аудио дорожках
                        _getAudio();
                        //Получаем информацию о встроенных субтитрах
                        _getSubtitle();
                        ...
                        _duration = _AVPlay.getDuration() + _correctionTime * 1000;
                        // Посылаем собитие загрузки метаданных 
                        document.getElementById(_containerID).dispatchEvent(_loadedmetadata);
                        // Устанавливаем воспроизведение в оригинальном масштабе
                        setDisplayMethod(0);
                        break;
                    ...
                    case 14:
                        _nCurrentTime = parseInt(data) + _correctionTime * 1000;
                        if (typeof _callbackCurrentTime == 'function') {
                            _callbackCurrentTime(_nCurrentTime)
                        }
                        break;
                    // 15 AD_START
                    case 15:
